...
void computeColor(in vec3 ray_start, in vec3 ray_dir, out vec4 color)
{
  ...
  vec4 top_color = vec4(0.285, 0.46, 0.66, 1.0);
  vec4 bottom_color = vec4(0.44, 0.3, 0.6, 1.0);
...
void main ()
{
  ...
  if (t < 0.0) {
    // no sphere hit, get color of background
    vec4 color;
    ...
    for (float i = 0.0; i < num_of_spheres; i += 1.0) {
      ...
      sphereIntersection(point3, normalize(point3_dir), pos_rad.xyz, pos_rad.w, second_t);
    ...
    if (second_t >= 0.0) {
      vec3 point4 = point3 + second_t * point3_dir;
      vec3 normalized_point3_dir = normalize(point3_dir);
      vec3 normal_at_point4 = normalize(point4 - hit_sphere_pos_rad.xyz);

      vec3 point4_dir;
      refractionDirection( (1.0 / refraction_coef), normalized_point3_dir, normal_at_point4, point4_dir);

      computeColor(point4, normalize(point4_dir), refraction_color); // first behind sphere intersection
    }
