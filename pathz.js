...
class SphereHelper
{
   ...
   update(idx, dt)
   {
      ...
      var n = 8;
      // var r =  Math.cos(n * theta);
      var r = Math.sin(n * this.theta);
      this.pos[0] = r * Math.cos(this.theta);
      this.pos[1] = r * Math.sin(this.theta);

      // var R = 5,
      //     r = 3,
      //     d = 5;
      // x = (R - r) * Math.cos(this.theta) + d * Math.cos(this.theta * (R - r) / r);
      // y = (R - r) * Math.sin(this.theta) - d * Math.sin(this.theta * (R - r) / r);
...
function setupSpheres()
{
   ...
   for (var i = 0; i < numSpheres; ++i)
   {
      sh.radius = getRandom(0.012, 0.013);
      sh.rgb[0] = getRandom(0.2, 1);
      sh.rgb[1] = getRandom(0.4, 0.9);
      sh.rgb[2] = getRandom(0.6, 1);
      sh.theta = getRandom(0, Math.PI * 2);
      sh.toData(i);
   }
}
